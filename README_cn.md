...
## ONNX-to-SNN fork 实验记录

本 fork 正在新增一条面向预训练 ONNX 分类模型的转换路线：

```text
ONNX -> Canonical Graph IR -> 结构化 PyTorch ANN + SpikingJelly SNN
```

当前实验均使用 CIFAR-10，小样本评估口径为 `calib_size=256`、`eval_size=64`、`T=64`、`scale_mode=99.9%`。在 Apple Silicon 本地环境中，后续默认使用 PyTorch MPS 设备运行实验。

| 模型 | ONNX 重建 ANN accuracy | SNN accuracy | 相对 accuracy | 说明 |
| --- | ---: | ---: | ---: | --- |
| ResNet18 CIFAR-10 | 0.9375 | 0.90625 | 0.9667 | 达到 `SNN >= 0.95 * ANN` 的阶段目标 |
| VGG16-BN CIFAR-10，保留 MaxPool | 0.921875 | 0.90625 | 0.9831 | 达到阶段目标 |
| VGG16-BN CIFAR-10，SNN 中事后将 MaxPool 直接替换为 AvgPool | 0.921875 | 0.09375 | 0.1017 | 不达到阶段目标 |

VGG16-BN 的对照实验显示：ONNX 到结构化 ANN 的数值重建是准确的，ONNXRuntime 与重建 ANN 的最大绝对误差为 `3.81e-06`，但对已经训练好的 VGG16-BN 权重，事后把 5 个 `MaxPool` 直接替换为 `AvgPool` 会先破坏代理 ANN 本身的分类能力，代理 ANN accuracy 仅为 `0.109375`。因此，准确率下降主要来自池化算子替换本身，而不是 ONNX 重建或 IFNode 替换。

推荐做法：

- 对已经训练好的 VGG/MaxPool 模型，默认保留 `MaxPool` 进行 ANN-to-SNN 转换，先保证可复现的 95% 相对性能。
- 若必须使用 `AvgPool`，应在 ANN 阶段就采用 AvgPool 结构训练，或在替换 MaxPool 后进行微调/再校准；不建议直接对已训练 MaxPool 权重做事后硬替换。
- 下一阶段工作是 pattern grouping：将 ONNX 拓扑图中的 `Conv-BN-ReLU`、VGG stage、ResNet residual block 等识别为更适合阅读和魔改的结构化模块。

## 更新日志

我们正在积极维护和改进 SpikingJelly。本节汇总当前项目状态与近期更新。
...
- 已添加`memopt`；
- `op_counter`现已支持 FLOPs、访存、SynOp 与解析式运行时能耗分析；
- 已添加`nir_exchange`；
...
注意，SpikingJelly是基于PyTorch的。安装SpikingJelly前，请先确保环境中已经安装了[PyTorch、torchvision和torchaudio](https://pytorch.org)。最新版的SpikingJelly依赖 `python>=3.11` 与 `torch>=2.6.0`；所有测试均在`torch==2.7.1`上进行。
...
已知的使用惊蜇(SpikingJelly)的出版物可见于[文档页面](https://spikingjelly.readthedocs.io/zh_CN/latest/publications.html)。权威数据源为[publications.json](./publications.json)。如果你的文章也使用了惊蜇(SpikingJelly)，可以通过提交 pull request 的方式更新该数据文件。
